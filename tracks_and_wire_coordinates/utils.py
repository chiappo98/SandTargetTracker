...
def sigma_fit(clY_z, pos, m_fit, q_fit):
    return np.sqrt(np.sum((pos - m_fit*clY_z - q_fit)**2)/(pos.size - 2))
...
def find_best_fit(clY_z, pos, m, q, sigma):
    ...
    if sigma > 0.2:
        for i in range(5):
            ...
            tmp_m = interpole_m(y, p)
            tmp_q = interpole_q(y, p, tmp_m)
            tmp_sigma = sigma_fit(y, p, tmp_m, tmp_q)#np.sqrt(np.sum((p - tmp_m*y - tmp_q)**2)/(4-2))
            if tmp_sigma < new_fit_par[2]:
